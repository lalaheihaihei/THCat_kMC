...
def get_react_rate(kmc, i, T):
    order_of_the_react = kmc.reactions[:i][1].count('react')  # to judge which three freq need to be input.
    input_freq = []
    for j in [order_of_the_react*3 + 0, order_of_the_react*3 + 1, order_of_the_react*3 + 2]:
        input_freq.append(kmc.Freq[j])
    # print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    # print(kmc.reactions[i][2], kmc.reactions[i][0], T, input_freq)
    react = reactions.Reactions(kmc.reactions[i][2], kmc.reactions[i][0], T, input_freq)
